...
function generarCartaResponsiva($folioCarta, $usuarioId, $equiposIds, $solicitud) {
    ...
    // Obtener datos del gerente que aprobó (puede ser Admin o General)
    $gerenteId = $solicitud['aprobado_por_general'] ?? $solicitud['aprobado_por'];
    ...
    // ⭐ USAR LA FUNCIÓN HELPER PARA FECHAS
    $fechaAprobacion = $solicitud['fecha_aprobacion_general'] ?? $solicitud['fecha_aprobacion'];
    $section->addText('Aprobado por:', ['size' => 10]);
    $section->addText(($gerente['nombre_completo'] ?? 'N/A') . ' - Fecha: ' . formatearFechaSQL($fechaAprobacion), ['size' => 9]);
    $section->addTextBreak(1);
