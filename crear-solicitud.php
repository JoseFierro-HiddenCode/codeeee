...
// Solo usuarios que son jefes pueden crear solicitudes
...
// Verificar que el colaborador sea del jefe
$sqlVerificar = "SELECT id FROM users WHERE id = ? AND jefe_id = ?";
$colaborador = obtenerUno($sqlVerificar, [$colaborador_id, $_SESSION['user_id']]);

if (!$colaborador) {
    header('Location: ../../views/inventario/solicitar-equipos.php?error=' . urlencode('El colaborador no pertenece a tu equipo'));
    exit();
}
...
function generarExcelDesdePlantilla($sol) {
    ...
    $reemplazos = [
        ...
        '${SP_NIVEL3}' => ($sol['sp_nivel'] == 'nivel_3') ? 'X' : '',
        '${SP_NIVEL4}' => ($sol['sp_nivel'] == 'nivel_4') ? 'X' : '',
        '${SP_NIVEL5}' => ($sol['sp_nivel'] == 'nivel_5') ? 'X' : '',
        '${SP_NIVEL6}' => ($sol['sp_nivel'] == 'nivel_6') ? 'X' : '',
