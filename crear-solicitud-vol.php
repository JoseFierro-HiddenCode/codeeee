...
// Jefes o admin_tecnico
protegerPagina(['usuario', 'admin_tecnico', 'admin_global']);

// Verificar permisos
$esAdmin = in_array($_SESSION['rol'], ['admin_tecnico', 'admin_global']);
$esJefeUsuario = isset($_SESSION['rol']) && $_SESSION['rol'] === 'usuario' && esJefe();

if (!$esAdmin && !$esJefeUsuario) {
...
// Verificar que el empleado pertenece al jefe (O es admin_tecnico)
if ($esAdmin) {
    // Admin puede crear para cualquier usuario activo
    $sqlVerificar = "SELECT id FROM users WHERE id = ? AND activo = 1";
    $empleadoValido = obtenerUno($sqlVerificar, [$empleado_id]);
} else {
    // Jefe solo puede crear para su equipo
...
if (!$empleadoValido) {
    $mensaje = $esAdmin ? 'El empleado no existe o está inactivo' : 'El empleado no pertenece a tu equipo';
    header('Location: ../../views/accesos/solicitud-accesos-vol.php?error=' . urlencode($mensaje));
    exit();
}
