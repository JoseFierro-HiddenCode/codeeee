...
function badgeEstadoSolicitud($estado) {
    $badges = [
        'pendiente_gerente' => '<span class="badge bg-warning">⏳ Pendiente Aprobación</span>',
...
function marcarEquipoNoDisponible($equipoId) {
    ...
    return ejecutarQuery($sql, [$equipoId]);
}
...
function equipoEstaDisponible($equipoId) {
    ...
    // Verificar que el resultado no sea null/false antes de acceder al índice
    if (!$result || !is_array($result)) {
        return false;
    }
    
    // Verificar que exista la clave 'disponible'
    if (!array_key_exists('disponible', $result)) {
        return false;
    }
    
    return (int)$result['disponible'] === 1;
}

/**
 * Obtener carta responsiva de un usuario (última asignación)
 */
...
/**
 * Obtener todos los equipos de una asignación por folio
 */
function obtenerEquiposPorFolioCarta($folioCarta) {
    $sql = "
        SELECT 
            e.*,
            a.fecha_asignacion
