...
/**
 * Proteger página por equipo (Soporte vs Desarrollo)
 */
function protegerPorEquipo($equipoRequerido) {
    global $conn;
    ...
    // Verificar que sea admin_tecnico o tecnico
    if (!in_array($_SESSION['rol'], ['admin_tecnico', 'tecnico'])) {
        redirigir('/sistema-tickets/index.php');
    }
    
    // Obtener equipo del usuario
    $sql = "SELECT equipo FROM users WHERE id = ?";
    $result = obtenerUno($sql, [$_SESSION['user_id']]);
    
    if (!$result || $result['equipo'] !== $equipoRequerido) {
        redirigir('/sistema-tickets/views/dashboard-admin.php');
    }
}

/**
 * Obtener técnicos por equipo
 */
function obtenerTecnicosPorEquipo($equipo) {
    $sql = "
        SELECT id, nombre + ' ' + apellido as nombre_completo
        FROM users
        WHERE rol IN ('tecnico', 'admin_tecnico') 
          AND activo = 1
          AND equipo = ?
        ORDER BY nombre_completo
    ";
    return obtenerTodos($sql, [$equipo]);
}

/**
 * Badge para equipo
 */
function badgeEquipo($equipo) {
    $badges = [
        'soporte' => '<span class="badge bg-primary">🛠️ Soporte Técnico</span>',
        'desarrollo' => '<span class="badge bg-success">💻 Desarrollo</span>',
    ];
    return $badges[$equipo] ?? '<span class="badge bg-secondary">' . e($equipo) . '</span>';
}

/**
 * Verificar si un técnico pertenece a un equipo
 */
function tecnicoPertenece($tecnicoId, $equipo) {
    $sql = "SELECT equipo FROM users WHERE id = ? AND rol IN ('tecnico', 'admin_tecnico')";
    $result = obtenerUno($sql, [$tecnicoId]);
    return ($result && $result['equipo'] === $equipo);
}

// ============================================
// OBTENER URL DEL DASHBOARD SEGÚN ROL Y EQUIPO
// ============================================
function obtenerDashboardUrl($relativa = false) {
    iniciarSesion();
    
    $prefix = $relativa ? '../' : '';
    $dashboardUrl = $prefix . 'dashboard-usuario.php'; // Default
    
    $rol = $_SESSION['rol'] ?? '';
    $equipo = $_SESSION['equipo'] ?? null;
    
    switch ($rol) {
        case 'gerente':
            $dashboardUrl = $prefix . 'dashboard-gerente.php';
            break;
            
        case 'gerente_general':
            $dashboardUrl = $prefix . 'dashboard-gerente-general.php';
            break;
            
        case 'admin_tecnico':
            // Redirigir según el equipo
            if ($equipo === 'desarrollo') {
                $dashboardUrl = $prefix . 'dashboard-admin-desarrollo.php';
            } elseif ($equipo === 'soporte') {
                $dashboardUrl = $prefix . 'dashboard-admin-soporte.php';
            } else {
                $dashboardUrl = $prefix . 'dashboard-admin.php';
            }
            break;
            
        case 'tecnico':
    // Técnicos tienen dashboards separados por equipo
    if ($equipo === 'desarrollo') {
        $dashboardUrl = $prefix . 'dashboard-tecnico-desarrollo.php';
    } elseif ($equipo === 'soporte') {
        $dashboardUrl = $prefix . 'dashboard-tecnico.php';
    } else {
        $dashboardUrl = $prefix . 'dashboard-tecnico.php';
    }
    break;
        ...
        case 'usuario':
        default:
            $dashboardUrl = $prefix . 'dashboard-usuario.php';
            break;
    }
...
function esAdminGlobal() {
    iniciarSesion();
    return isset($_SESSION['rol']) && $_SESSION['rol'] === 'admin_global';
}

// ============================================
// VERIFICAR SI ES USUARIO AUTORIZADO ESPECIAL
// ============================================
function esUsuarioAutorizadoEspecial() {
    // IDs de usuarios con permisos especiales para solicitar equipos
    $usuariosAutorizados = [
        20, 
    ...
    $userId = $_SESSION['user_id'] ?? 0;
    return in_array($userId, $usuariosAutorizados);
}
?>