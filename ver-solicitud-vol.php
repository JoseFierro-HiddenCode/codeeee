...
// Permitir si es jefe O si es admin/técnico
$esAdmin = in_array($_SESSION['rol'] ?? '', ['admin_tecnico', 'admin_global']);
$esJefeValido = esJefe();

if (!$esJefeValido && !$esAdmin) {
    header('Location: ../dashboard-usuario.php?error=' . urlencode('No autorizado: debe ser jefe o administrador'));
    exit();
}
$folio = $_GET['folio'] ?? '';
