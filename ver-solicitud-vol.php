...
<a href="<?= $rutaDescarga ?>" 
   class="btn <?= $colorBtn ?> btn-lg">
    ...
    <!-- DEBUG: Ver qué archivo está buscando -->
<div class="alert alert-secondary mt-3">
    <small>
        <strong>DEBUG:</strong><br>
        Folio: <?= $solicitud['folio'] ?><br>
        Archivo buscado: <?= $nombreArchivo ?><br>
        Ruta completa: <?= $rutaCompleta ?><br>
        ¿Existe?: <?= file_exists($rutaCompleta) ? '✅ SÍ' : '❌ NO' ?><br>
        <?php if (file_exists($rutaCompleta)): ?>
            Tamaño: <?= filesize($rutaCompleta) ?> bytes<br>
            Última modificación: <?= date('Y-m-d H:i:s', filemtime($rutaCompleta)) ?>
        <?php endif; ?>
    </small>
</div>
