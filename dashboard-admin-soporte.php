...
<div class="sidebar-section-title">Accesos Volvo</div>
<ul class="sidebar-nav">
    <li class="sidebar-nav-item">

    
        <a href="accesos/solicitudes-vol-ti.php" class="sidebar-nav-link">
            <i class="bi bi-inbox"></i>
            <span>Solicitudes Pendientes</span>
            <?php
            $sqlVolPendientes = "SELECT COUNT(*) as total FROM solicitudes_accesos_vol_historial WHERE estado = 'pendiente'";
            $volPendientes = obtenerUno($sqlVolPendientes);
            if ($volPendientes && $volPendientes['total'] > 0):
            ?>
                <span class="ms-auto badge bg-warning"><?php echo $volPendientes['total']; ?></span>
            <?php endif; ?>
        </a>
    </li>
    <li class="sidebar-nav-item">
    <a href="accesos/consultar-accesos-vol.php" class="sidebar-nav-link">
        <i class="bi bi-search"></i>
        <span>Consultar Accesos</span>
    </a>
</li>

<li class="sidebar-nav-item">
        <a href="accesos/solicitud-accesos-vol.php" class="sidebar-nav-link">
            <i class="bi bi-person-plus-fill"></i>
            <span>Crear Solicitud para Usuario</span>
        </a>
    </li>
</ul>
                    ...
                    <?php else: ?>
                        <div class="mt-2">
                           <button class="btn btn-sm btn-primary w-100" 
                            onclick="event.stopPropagation(); abrirModalAsignar(<?php echo $ticket['id']; ?>, '<?php echo htmlspecialchars($ticket['tecnico_nombre'] ?? '', ENT_QUOTES); ?>', '<?php echo $ticket['tecnico_asignado_id'] ?? ''; ?>')">
