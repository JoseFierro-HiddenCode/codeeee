...
protegerPagina(['admin_tecnico', 'tecnico', 'admin_global']);
...
$sql = "SELECT e.*, s.nombre as sede_nombre FROM equipos e LEFT JOIN sedes s ON e.sede_id = s.id WHERE 1=1";
$params = [];
...
<body>
    ...
    <nav class="navbar navbar-expand-lg navbar-custom">
        <div class="container-fluid">
            <a class="navbar-brand d-flex align-items-center" href="<?php echo obtenerDashboardUrl(true); ?>">
            ...
            <table style="width: 100%; border-collapse: collapse; font-size: 14px;">
                <thead>
                    <tr>
                        ...
                        <th style="background-color: var(--bg-tertiary); color: var(--text-muted); font-size: 11px; font-weight: 700; text-transform: uppercase; padding: 12px 16px; border: none; border-bottom: 2px solid var(--border-color); text-align: left; width: 130px;">SEDE</th>
                ...
                <tbody>
                    ...
                    <tr style="transition: background-color 0.2s ease;"
                       ...
                       <td style="padding: 16px; border: none; border-bottom: 1px solid var(--border-color); color: var(--text-secondary); vertical-align: middle;">
                            <?php echo e($equipo['sede_nombre'] ?: 'Sin asignar'); ?>
                        </td>
...
<div class="modal fade" id="modalNuevoEquipo" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <form action="../../actions/inventario/crear-equipo.php" method="POST">
                <div class="modal-body">
                    
                    <!-- SECCIÓN: UBICACIÓN FÍSICA -->
                    <div class="mb-4">
                        <h6 class="text-primary mb-3">
                            <i class="bi bi-building"></i> Ubicación Física
                        </h6>
                        <div class="row">
                            <div class="col-md-12 mb-3">
                                ...
                                <select class="form-select" id="sede_id" name="sede_id" required>
                                    ...
                                    <?php
                                    $sedes = obtenerTodos("SELECT id, nombre FROM sedes WHERE activo = 1 ORDER BY nombre");
                                ...
                                <small class="text-muted">¿En qué sede se encuentra físicamente este equipo?</small>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mb-4">
                        <h6 class="text-primary mb-3">
                            <i class="bi bi-laptop"></i> Información del Equipo
                        </h6>
                        <div class="row">
                            
                            <!-- TIPO DE EQUIPO -->
                            <div class="col-md-6 mb-3">
                                <label for="tipo-nuevo" class="form-label">
                                    Tipo de Equipo <span class="text-danger">*</span>
                                </label>
                                <select class="form-select" id="tipo-nuevo" name="tipo" required>
                            ...
                            <!-- MARCA -->
                            <div class="col-md-6 mb-3" id="campo-marca">
                                <label for="marca" class="form-label">
                                    Marca <span class="text-danger campo-requerido">*</span>
                                </label>
                            ...
                            <!-- MODELO -->
                            <div class="col-md-6 mb-3" id="campo-modelo">
                                <label for="modelo" class="form-label">
                                    Modelo <span class="text-danger campo-requerido">*</span>
                                </label>
                            ...
                            <div class="col-md-6 mb-3">
                                <label for="numero_serie" class="form-label">
                                    Número de Serie <span class="text-danger campo-requerido">*</span>
                                </label>
                                <input type="text" class="form-control" id="numero_serie" name="numero_serie" 
                                       placeholder="Se generará automáticamente si se deja vacío">
                                <small class="text-muted">
                                    <i class="bi bi-info-circle"></i> 
                                    Déjalo vacío para generar automáticamente según sede y tipo
                                </small>
                            </div>

                            <!-- PROCESADOR -->
                            <div class="col-md-6 mb-3" id="campo-procesador" style="display: none;">
                            ...
                            <!-- SISTEMA OPERATIVO -->
                            <div class="col-md-6 mb-3" id="campo-sistema-operativo" style="display: none;">
                    ...
                    <!-- SECCIÓN: ASIGNACIÓN INICIAL (OPCIONAL) -->
                    <div class="mb-4">
                        <h6 class="text-primary mb-3">
                            <i class="bi bi-person-check"></i> Asignación Inicial (Opcional)
                        </h6>
                        <div class="row">
                            <div class="col-md-12 mb-3">
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="estado_asignacion" 
                                           id="disponible" value="disponible" checked>
                                    <label class="form-check-label" for="disponible">
                                        <i class="bi bi-box-seam text-success"></i> 
                                        <strong>Disponible</strong> - Guardar en almacén
                                    </label>
                                </div>
                                <div class="form-check mt-2">
                                    <input class="form-check-input" type="radio" name="estado_asignacion" 
                                           id="asignado" value="asignado">
                                    <label class="form-check-label" for="asignado">
                                        <i class="bi bi-person-fill text-primary"></i> 
                                        <strong>Asignado</strong> - Asignar directamente a un usuario
                                    </label>
                                </div>
                            </div>

                            <!-- CAMPOS DE ASIGNACIÓN (OCULTOS POR DEFECTO) -->
                            <div id="campos-asignacion" style="display: none;">
                                <div class="col-md-12 mb-3">
                                    <label for="usuario_asignado" class="form-label">
                                    ...
                                    <select class="form-select" id="usuario_asignado" name="usuario_asignado">
                                        ...
                                        SELECT u.id, u.nombre, u.apellido, s.nombre as sede_nombre, u.sede_id
                                        FROM users u
                                        ...
                                        ?>
                                            <option value="<?php echo $user['id']; ?>" data-sede="<?php echo $user['sede_id']; ?>">
                                                <?php echo e($user['nombre'] . ' ' . $user['apellido']); ?> 
                                                - <?php echo e($user['sede_nombre'] ?? 'Sin sede'); ?>
                                            </option>
                                        <?php endforeach; ?>
                                    </select>
                                </div>
                                ...
                                <div class="col-md-12 mb-3">
                                    <label for="notas_asignacion" class="form-label">Notas de Asignación</label>
                                    <textarea class="form-control" id="notas_asignacion" name="notas_asignacion" 
                                              rows="2" placeholder="Motivo o detalles de la asignación..."></textarea>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <!-- ✅ PRIMERO: Cargar Bootstrap -->
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js"></script>

    <!-- ✅ SEGUNDO: Cargar app.js (tema oscuro) -->
    <script src="../../public/js/app.js"></script>

    <!-- ✅ TERCERO: Script de campos dinámicos (DESPUÉS de Bootstrap) -->
    <script>
    // ============================================
    // CAMPOS OBLIGATORIOS DINÁMICOS + ASIGNACIÓN
    // ============================================
    document.addEventListener('DOMContentLoaded', function() {
        const tipoSelect = document.getElementById('tipo-nuevo');
        const campoProcesador = document.getElementById('campo-procesador');
        const campoSO = document.getElementById('campo-sistema-operativo');
        const campoMarca = document.getElementById('campo-marca');
        const campoModelo = document.getElementById('campo-modelo');
        
        // Radio buttons de asignación
        const radioDisponible = document.getElementById('disponible');
        const radioAsignado = document.getElementById('asignado');
        const camposAsignacion = document.getElementById('campos-asignacion');
        const usuarioSelect = document.getElementById('usuario_asignado');
        
        if (!tipoSelect) return;
        
        // ========================================
        // FUNCIÓN: ACTUALIZAR CAMPOS OBLIGATORIOS
        // ========================================
        function actualizarCamposObligatorios() {
            const tipo = tipoSelect.value;
            
            // Tipos computacionales
            const tiposComputacionales = ['Laptop', 'PC de Escritorio', 'Tablet', 'Celular'];
            
            // Tipos de periféricos
            const tiposPerifericos = ['Mouse', 'Teclado', 'Audífonos', 'Webcam', 'Bocinas', 'Micrófono', 
                                      'Monitor', 'Impresora', 'Escáner', 'Proyector'];
            
            // Tipos de cables y accesorios (mínimos requerimientos)
            const tiposCablesAccesorios = ['Cable HDMI', 'Cable VGA', 'Cable DisplayPort', 'Cable USB', 
                                           'Cable USB-C', 'Cable de Red', 'Cable de Poder', 'Cable de Audio',
                                           'Cargador de Laptop', 'Cargador de Celular', 'Adaptador HDMI',
                                           'Adaptador VGA', 'Adaptador USB-C', 'Hub USB', 'Docking Station',
                                           'Extensión Eléctrica', 'Regulador'];
            
            const esComputacional = tiposComputacionales.includes(tipo);
            const esPerifenico = tiposPerifericos.includes(tipo);
            const esCableAccesorio = tiposCablesAccesorios.includes(tipo);
            
            // Limpiar todos los asteriscos
            document.querySelectorAll('.campo-requerido').forEach(el => el.style.display = 'none');
            
            // EQUIPOS COMPUTACIONALES: Marca, Modelo, Serie obligatorios
            if (esComputacional) {
                campoProcesador.style.display = 'block';
                campoSO.style.display = 'block';
                campoMarca.querySelector('.campo-requerido').style.display = 'inline';
                campoModelo.querySelector('.campo-requerido').style.display = 'inline';
                document.getElementById('marca').required = true;
                document.getElementById('modelo').required = true;
            }
            // PERIFÉRICOS: Solo Marca obligatoria
            else if (esPerifenico) {
                campoProcesador.style.display = 'none';
                campoSO.style.display = 'none';
                campoMarca.querySelector('.campo-requerido').style.display = 'inline';
                campoModelo.querySelector('.campo-requerido').style.display = 'none';
                document.getElementById('marca').required = true;
            ...
            // CABLES/ACCESORIOS: Todo opcional (se genera automáticamente)
            else if (esCableAccesorio) {
                campoProcesador.style.display = 'none';
                campoSO.style.display = 'none';
                campoMarca.querySelector('.campo-requerido').style.display = 'none';
                campoModelo.querySelector('.campo-requerido').style.display = 'none';
            ...
            // OTROS: Marca y Modelo obligatorios
            else {
                campoProcesador.style.display = 'none';
                campoSO.style.display = 'none';
                campoMarca.querySelector('.campo-requerido').style.display = 'inline';
        ...
        // ========================================
        // FUNCIÓN: MOSTRAR/OCULTAR ASIGNACIÓN
        // ========================================
        function toggleCamposAsignacion() {
            if (radioAsignado && radioAsignado.checked) {
                camposAsignacion.style.display = 'block';
                usuarioSelect.required = true;
            } else {
                camposAsignacion.style.display = 'none';
                usuarioSelect.required = false;
                if (usuarioSelect) usuarioSelect.value = '';
                const notasAsignacion = document.getElementById('notas_asignacion');
                if (notasAsignacion) notasAsignacion.value = '';
            }
        }
        ...
        // EVENTOS
        // ========================================
        tipoSelect.addEventListener('change', actualizarCamposObligatorios);
        tipoSelect.addEventListener('input', actualizarCamposObligatorios);
        
        if (radioDisponible) radioDisponible.addEventListener('change', toggleCamposAsignacion);
        if (radioAsignado) radioAsignado.addEventListener('change', toggleCamposAsignacion);
        ...
        if (modal) {
            modal.addEventListener('show.bs.modal', function () {
                ...
                // Resetear asignación
                if (radioDisponible) radioDisponible.checked = true;
                camposAsignacion.style.display = 'none';
                usuarioSelect.required = false;
            });
        }
